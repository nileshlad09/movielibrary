...
const Upcoming = (props) => {
  ...
  const pageChange= async(pageNo)=>{
    if(movieN){
      API_URL = BASE_URL + "/search/tv?" + API_KEY + `&query=${movieN}&page=${pageNo}`;  
    }
    else{
  ...
  useEffect(() => {    
     API_URL =
     BASE_URL + "/tv/popular?" + API_KEY + `&page=1`;
     fetchFun(API_URL)
  }, []);

  const nowPlaying =()=>{
    API_URL =
    BASE_URL + "/tv/airing_today?" + API_KEY + `&page=1`;
    let now= 1;
  ...
  const topRated=()=>{
    API_URL =
    BASE_URL + "/tv/top_rated?" + API_KEY + `&page=1`;
    let now= 4;
  ...
  return (
    ...
    <div className="main">
      ...
      <div className="movieContainer">
        {movie.length > 0 &&
          movie.map((movie) => {
            return <TvshowItem key={movie.id} movie={movie} showAlert={showAlert}/>;
          })}
        <h3 style={{display:(movie.length<1)?"block":"none",color:"white"}}>Series Not Found</h3>
      </div>
      </div>
